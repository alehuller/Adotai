...
public interface OngControllerDocs {
    @Operation(summary = "Retorna todas as ONGs", responses = {
        @ApiResponse(responseCode = "200", description = "Success",
            content = @Content(mediaType = "application/json",
                array = @ArraySchema(schema = @Schema(implementation = OngDTO.class)))),
    ...
    ResponseEntity<?> deletarOngPorId(Long id);

    @Operation(summary = "Retorna todas as adoções de uma ONG", responses = {
        @ApiResponse(responseCode = "200", description = "Success",
            content = @Content(mediaType = "application/json",
                array = @ArraySchema(schema = @Schema(implementation = AdocaoDTO.class)))),
        @ApiResponse(responseCode = "400", description = "Bad Request", content = @Content),
        @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content),
        @ApiResponse(responseCode = "404", description = "Not Found", content = @Content),
        @ApiResponse(responseCode = "500", description = "Internal Error", content = @Content)
    })
    ResponseEntity<PagedModel<EntityModel<AdocaoDTO>>> listarAdocoesPorOngId(Long id, int page, int size, String direction);
    ...
    ResponseEntity<OngDTO> atualizarParcialOng(Long id, Map<String, Object> updates);
}
